...
账本是 JSON Lines，一个文件一行一个 JSON 对象。持仓类不读取、不记录 markdown。

- 真实持仓：`portfolio/holdings.jsonl`。未指明账本，或说「我的持仓」，用这本。
- 模拟仓：`portfolio/simulated/holdings.jsonl`。只有「你的持仓」用这本。
- 其他仓：节点是空的。不建第三本账，也不做注册表。以后真有第三本，再增加一个 `.jsonl` 路径，仍用同一个类。
...
`jsonl_path` 是这本账的身份，需要留下来。`load()` 的结果不存成属性。`book()` 里的 `found` 要检查条数，所以留下。
...
`portfolio/collector.py` 里的 `load_holdings()` 今天仍读 `holdings.json`。采集和评委这次不改。

持仓类落地时，只做这些：

- 按上面的行格式，从现有 JSON 写出两份 `.jsonl`。`HOLDINGS.md` 留在仓库里，但不是账本，类不指向它。
- 新增 `Holdings`、`REAL`、`SIMULATED`、`open_holdings`。
- 不把 `load_holdings()` 改成读 JSON Lines。那会碰到采集流程，等单独确认。
