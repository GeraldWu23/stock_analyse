---
name: fetch-quotes
description: 拉取 A 股 / 港股 / ETF 的实时行情(现价、涨跌、涨跌幅、最高、最低、成交量)。当用户询问股票/标的价格、行情、盘中报价,或提到自选(南方航空A、太古地产H、兆易创新H、大陆ETF)时使用。数据源腾讯优先、新浪兜底。
---
...
```bash
# 拉取内置自选(南方航空A / 太古地产H / 兆易创新H / 大陆ETF)
python fetch_quotes.py

# 指定代码(前缀:上交所 sh、深交所 sz、港股 hk 补足 5 位)
python fetch_quotes.py sh600029 hk01972 hk03986 sh510300
